...
@php
    $user = Auth::user();
    $notifications = \App\Models\Cuti::where('user_id', $user->id)->latest()->take(50)->get();
    $notificationCount = \App\Models\Cuti::where('user_id', $user->id)->whereIn('status', ['disetujui', 'ditolak'])->count();
@endphp
<div class="app-shell">
    ...
    <aside class="sidebar">
        <div class="sidebar-brand">
            ...
            <div>
                <div class="fw-bold">Hotel Leave</div>
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('pegawai.cuti.index') }}" class="{{ request()->routeIs('pegawai.cuti.*') ? 'active-link' : '' }}">
                <i class="fa fa-calendar-days"></i> Cuti Saya
            </a>
            <a href="{{ route('profile.edit') }}" class="{{ request()->routeIs('profile.*') || request()->routeIs('pegawai.profile') ? 'active-link' : '' }}">
            ...
            <div class="d-flex align-items-center gap-2">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-end p-0 shadow notification-menu">
                        <div class="px-3 py-2 border-bottom fw-bold notification-header">Notifikasi Cuti Saya</div>
                        <div class="notification-list">
                        @forelse($notifications as $notif)
                            <a class="dropdown-item py-2 notification-item" href="{{ route('pegawai.cuti.index') }}">
                                <div class="fw-semibold">{{ $notif->jenis_cuti ?? 'Cuti' }} - {{ ucfirst($notif->status) }}</div>
                                <small class="text-muted">{{ optional($notif->created_at)->format('d M Y H:i') }}</small>
                            </a>
                        @empty
